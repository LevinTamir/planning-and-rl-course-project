...
class RLEnvironment(Node):
    ...
    def call_task_succeed(self):
        while not self.task_succeed_client.wait_for_service(timeout_sec=1.0):
            self.get_logger().warn(
                "service for task succeed is not available, waiting ..."
            )
    ...
    def call_task_failed(self):
        while not self.task_failed_client.wait_for_service(timeout_sec=1.0):
            self.get_logger().warn(
                "service for task failed is not available, waiting ..."
            )
    ...
    def odom_sub_callback(self, msg):
        ...
        path_theta = math.atan2(
            self.goal_pose_y - self.robot_pose_y, self.goal_pose_x - self.robot_pose_x
        )
